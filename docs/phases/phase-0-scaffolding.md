...
- Trên Windows, npm chạy script bằng `cmd.exe` — path executable trong `package.json` scripts phải dùng backslash (`backend\.venv\Scripts\python.exe`), dùng forward-slash sẽ bị lỗi "not recognized as an internal or external command".
- Template shadcn-admin có sẵn `@clerk/react` (dịch vụ auth bên thứ 3, cần tài khoản Clerk) — không cần cho tool cá nhân chạy local. **Cần gỡ ở đầu Phase 1**: xoá `src/features/auth` liên quan Clerk, `src/routes/clerk`, dependency `@clerk/react` trong `package.json`, thay bằng không-auth hoặc auth đơn giản riêng (tuỳ theo câu hỏi multi-tenant ở Phase 7).
