...
## Tạm tắt lấy video theo kênh (2026-09-24)
Bilibili chặn risk-control (412) gần như 100% request `space/wbi/arc/search` không đăng nhập → chỉ tốn request và nhồi nhật ký. Cờ `FEATURE_CHANNEL_VIDEOS = false` ở `frontend/src/config/app.ts`: ẩn dải "Video khác trong kênh" ở popup xem trước (không gọi API), tab "Kênh đã theo dõi" vẫn liệt kê kênh đã theo dõi nhưng chọn kênh chỉ hiện thông báo tạm tắt. Backend/route giữ nguyên; đặt cờ `true` để bật lại (test `channelVideosEnabled` đã phủ cả hai trạng thái). Theo dõi/bỏ theo dõi kênh và dải "Video tương tự" vẫn hoạt động.
