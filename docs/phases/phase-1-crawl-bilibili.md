...
### Phiên 2026-09-16 — cải thiện chất lượng trang Trending (chuyên mục, chart, video xu hướng)

**Lý do:** người dùng phản hồi 3 phần đã hoạt động về mặt tính năng nhưng "chưa ổn": hiển thị chuyên mục, biểu đồ "chủ đề đang được quan tâm", và danh sách "video đang lên xu hướng". Deep research phát hiện nguyên nhân gốc cho cả 3, verify bằng request thật:

1. **Chuyên mục bị đoán nhóm sai** — cơ chế cũ (`_GROUP_HINTS`, khớp từ khoá tiếng Trung) xếp "Hài hước" (rid 138) vào "Giải trí", nhưng bảng phân khu CHÍNH THỨC của Bilibili (đối chiếu [github.com/pskdje/bilibili-API-collect](https://github.com/pskdje/bilibili-API-collect), bản đồng bộ từ repo gốc SocialSisterYi trước khi bị gỡ) xác nhận 138 thuộc phân khu 生活 (Đời sống). Verify chéo với DB thật: bug này tồn tại thật trong `categories` table.
2. **Video "đang xu hướng" bỏ phí tín hiệu tốt nhất** — gọi thật `ranking/region` phát hiện field `pts` (điểm xếp hạng THẬT Bilibili tự tính) chưa được dùng; verify thứ tự `pts` giảm dần khớp đúng thứ hạng trả về kể cả khi lượt xem thô thì không. Cũng phát hiện trang 2+ (khi cuộn thêm) âm thầm đổi nguồn từ ranking thật sang search từ khoá chung chung — không còn đúng nghĩa "xu hướng" nhưng UI trước đó nối liền mạch, không phân biệt.
3. **Chart dùng chỉ số dễ lệch** — cộng tổng `play` (view thô tích luỹ cả đời video) của ~9-11 video đang rank, dễ bị 1 video viral cũ làm sai lệch cảm nhận "đang hot". Đổi sang tổng `pts`.

**Đã verify field mapping bằng request thật** (không đoán): gọi `x/web-interface/view` cho cùng 1 bvid, đối chiếu `stat.reply`/`stat.danmaku`/`stat.coin` với `review`/`video_review`/`coins` của `ranking/region` — xác nhận chính xác field nào là gì (tài liệu cộng đồng còn ghi chú nghi vấn "?" ở field này). Cũng verify `create` (ranking) là giờ Bắc Kinh UTC+8 bằng cách so khớp với `pubdate` (epoch) của cùng video, lệch đúng 8 tiếng.

**Đã làm:**
- `app/services/category_service.py`: bảng `_TID_GROUP` (tid → nhóm) tra cứu trực tiếp từ phân khu chính thức, thay vì đoán bằng từ khoá (giữ `_GROUP_HINTS` làm fallback cho tid quá mới, tài liệu chưa kịp cập nhật). Thêm `resync_known_groups()` — sửa lại nhóm cho MỌI chuyên mục đã có trong DB (không chỉ mục vừa quét thấy), gọi mỗi lần `discover_categories()` chạy.
- `TrendingVideoRead` + `_from_ranking_item`/`_from_search_item` (`trending_service.py`): thêm `comment_count`, `danmaku_count`, `coin_count`, `heat_score` (=pts, null nếu từ search), `published_at` (chuẩn hoá ISO UTC từ 2 định dạng khác nhau).
- `TrendingPageRead.source: "ranking" | "search"` — frontend dùng để chèn ngăn cách rõ trong lưới, không còn coi 2 nguồn là 1 danh sách.
- `CategoryStatsRead`/`CategorySnapshot`/`SnapshotPoint`: thêm `total_pts`, dùng làm chỉ số chính thay `total_plays` (cột DB mới qua `ensure_schema_columns()`).
- Frontend (`features/trending/`): card video hiện thêm bình luận/xu/"X ngày trước" + huy hiệu "🔥 Hot" khi có `heat_score`; chèn dòng ngăn cách "Duyệt thêm theo chuyên mục — không phải bảng xếp hạng..." trước video đầu tiên đến từ search; chart đổi sang `total_pts`.
- 6 test mới (`test_category_service.py`) cho bug cụ thể (138 → Đời sống) + cơ chế resync. Toàn bộ 394 test backend pass.
- **Verify bằng Playwright thật**: chạy `discover_categories()` thật → rid 138 đổi từ "Giải trí" sang "Đời sống" trong DB; card hiện đúng huy hiệu Hot/bình luận/xu/ngày đăng; dòng ngăn cách xuất hiện đúng vị trí ranh giới ranking/search; chart vẽ đường theo `total_pts`. Không lỗi console.
- **Sự cố vận hành trong lúc verify**: backend `uvicorn --reload` bị treo (đúng pattern đã ghi ở "Ghi chú" phiên trước — worker process cũ không thực sự reload dù log báo "Reloading"). Xử lý theo đúng quy trình đã ghi: tìm PID `--multiprocessing-fork` qua `Get-CimInstance Win32_Process`, kill sạch, khởi động lại. Nhắc người dùng: nếu quay lại thấy dev server không phản hồi, chạy lại `npm run dev` ở root.
...
### Phiên 2026-09-16 (tiếp) — dịch tự động chạy nền, tab "Tất cả", tìm kiếm tự do, loading spinner

Tiếp nối phiên cải thiện Trending cùng ngày (mục "Phiên 2026-09-16" ở trên). Người dùng phản hồi thêm sau khi xem ảnh chụp panel "Chuyên mục theo dõi": nhiều mục còn hiện tiếng Trung ở cả 2 cột, muốn (1) dịch tự động có cache/chỉ dịch chủ đề mới, (2) thêm tab "Tất cả", (3) ô tìm kiếm tự do, (4) loading animation khi chờ response.

**(1) Dịch tự động — phát hiện 2 vấn đề, không phải 1:**
- Bug thật: `translate_missing_names()` gọi `translate_service.translate_text()` (không cache) thay vì `translate_cached()` (cache bền trong bảng `translation_cache`, khoá theo hash nội dung — đã có sẵn, dùng cho tooltip dịch tiêu đề). Đã sửa dùng `translate_cached()`.
- Vấn đề chính không phải thiếu cache (đã có qua `name_vi` — mục dịch rồi thì bị filter `name_vi.is_(None)` loại ra ngay, đúng ý "chỉ dịch chủ đề mới") mà là **cơ chế kích hoạt**: dịch chỉ chạy khi bấm "Quét chuyên mục mới", giới hạn 20 mục/lần — quét ra 39 mục mới 1 lúc thì phải bấm lại nhiều lần mới dịch hết, và nếu không bấm thì backlog đứng yên mãi.
- **Không dùng file JSON** như đề xuất ban đầu của người dùng — DB (`Category.name_vi` + bảng `translation_cache`) đã là cache bền, còn phù hợp hơn JSON vì đồng bộ với cách các dữ liệu khác trong app lưu trữ (chuẩn bị đóng gói desktop, Phase 12). Đã giải thích rõ lý do thay vì làm theo yêu cầu chữ nghĩa.
- **Đã làm:** `_queue_translation_if_pending()` (`app/api/trending.py`) — cả `GET /categories` lẫn `POST /categories/refresh` giờ tự xếp `translate_missing_names()` chạy qua `BackgroundTasks` (không chặn response) mỗi khi còn mục chưa dịch (`category_service.count_pending_translations()` > 0, tránh spawn task thừa). Giới hạn nâng 20 → 100/lượt (không còn lý do giữ thấp khi chạy nền). Frontend: `useQuery` cho categories thêm `refetchInterval` — poll 8s trong lúc còn mục `name === name_zh` (chưa dịch), tự dừng khi hết — tên tiếng Việt tự hiện dần không cần thao tác gì thêm.
- **Verify:** gọi thật `GET /categories` qua backend đang chạy → log xác nhận background task chạy đúng (thử OpenAI trước, fallback Google). **Không dịch được lượt này** — nhưng vì lý do môi trường, không phải lỗi code: chưa cấu hình key OpenAI thật (401), và Google Translate free bị rate-limit (429) do phiên test gọi dồn dập cả ngày. Cơ chế retry-sau (mục thất bại vẫn `name_vi=None`, tự thử lại ở lượt gọi kế tiếp) đã đúng thiết kế — verify bằng 3 test mock (`TestTranslateMissingNames`, `TestCountPendingTranslations`) vì không ép được provider thật thành công lúc này. **Việc cần làm khi quay lại:** cấu hình 1 API key OpenAI thật trong trang API Keys, hoặc đợi Google free hết rate-limit (thường vài chục phút) rồi mở lại trang — nên tự dịch hết mà không cần thao tác gì.

**(2) Tab "Tất cả":** dùng thẳng `x/web-interface/popular` (đã có sẵn service `get_bilibili_popular`, chưa từng nối UI) — danh sách phổ biến TOÀN TRANG Bilibili, có phân trang thật (khác `ranking/region`). Đổi thành `get_bilibili_popular_page()` trả `TrendingPageRead` (`source="popular"`) để dùng chung khung hiển thị với category/search. Verify request thật: item có đủ `stat.reply/danmaku/coin/like` giống hệt endpoint chi tiết video — giàu hơn cả `ranking/region` (không mập mờ field `review`/`video_review`), chỉ thiếu `pts` (không phải bảng xếp hạng thuật toán, không gắn badge Hot cho nguồn này).

**(3) Tìm kiếm tự do:** endpoint mới `GET /api/trending/bilibili/search?keyword=&page=` (service `search_bilibili()`, tái dùng `client.search_videos()` + `_from_search_item()` đã có). Ô input ở đầu trang Trending, submit thì ẩn hẳn Tabs/Chart, hiện kết quả tìm kiếm + nút "Xoá tìm kiếm" quay lại.

**Refactor để dùng chung 1 khung hiển thị cho cả 3 nguồn (category/"Tất cả"/tìm kiếm):** `CategoryPanel` (nhận `rid`) đổi thành `VideoGridPanel` (nhận `queryKey` + `fetchPage`) — logic chọn video/xem trước/ngăn cách nguồn giữ nguyên, chỉ đổi cách lấy dữ liệu. Sửa luôn logic ngăn cách "search": chỉ hiện khi có ít nhất 1 trang KHÔNG PHẢI search trước đó (category: ranking→search mới hiện; "Tất cả"/tìm kiếm tự do: không có trang nào để "so", không hiện ngăn cách vô nghĩa).

**(4) Loading spinner:** thêm `Loader2` xoay (theo đúng pattern `size-3.5 animate-spin` đã dùng ở `batch-panel.tsx`/`generation-jobs-panel.tsx`...) vào nút "Quét chuyên mục mới", "Tải video đã chọn", và trạng thái tải ban đầu của lưới video — thay chữ đứng yên "Đang tải...".

**Verify Playwright thật:** tab "Tất cả" mặc định, có dữ liệu + stat thật; tìm "review dien thoai" ra kết quả gắn nhãn rõ "không phải bảng xếp hạng", nút xoá hoạt động, không có huy hiệu Hot/coin (đúng vì nguồn search); chart vẫn vẽ đúng sau khi chờ đủ lâu (lần đầu chụp sớm bị hiểu nhầm là "trống", thực ra chỉ đang tải nhiều request tuần tự). Không lỗi console. 4 test mới, 398 test backend pass.

### Phiên 2026-09-21 — gộp ô tìm kiếm thành component chung, Trending thiếu tuỳ chọn dịch

**Bạn phát hiện qua UI thật**: ô tìm kiếm tự do ở trang Trending không có tuỳ chọn "Dịch từ khoá sang tiếng Trung" như trang Crawl (`/crawl`, mục "Tạo job crawl") — và suy đoán đúng: cả 2 ô đều là JSX viết tay riêng biệt, không dùng chung component, nên tính năng thêm ở 1 nơi không tự lan sang nơi kia.

**Không chỉ thiếu UI — thiếu cả backend**: `GET /api/trending/bilibili/search` (dùng cho ô tìm Trending) chưa từng có tham số `translate_keyword`, khác hẳn `POST /api/jobs` (dùng cho Crawl) đã có từ trước. Nghĩa là trước phiên này, search tiếng Việt ở trang Trending gần như luôn ra kết quả rác/không liên quan (Bilibili search khớp mù theo chuỗi Latin, không tự dịch).

**Sửa tận gốc, không chỉ vá UI:**
- `trending_service.search_bilibili()` đổi chữ ký, nhận `db`/`user_id`/`translate_keyword` — tái dùng đúng `crawl_service.translate_keyword_to_chinese()` đã có sẵn cho Crawl (không viết lại logic dịch lần 2). Trả thêm field `translation_failed` (giống `job.translation_failed` của Crawl) để UI cảnh báo khi dịch lỗi thay vì âm thầm search sai.
- `GET /api/trending/bilibili/search` thêm query param `translate_keyword` (mặc định `false`, giữ hành vi cũ cho caller không truyền).
- Frontend: tạo `components/keyword-search-box.tsx` — component **dùng chung** (input + nút tìm + checkbox dịch + nút xoá tuỳ chọn), thay cho 2 form viết tay riêng ở `crawl/index.tsx` và `trending/index.tsx`. Trending giờ có checkbox dịch (mặc định bật, giống Crawl) + toast cảnh báo khi dịch thất bại (đọc `translation_failed` từ trang đầu kết quả trong `VideoGridPanel`).
- Test mới: 3 test `search_bilibili` (dịch trước khi search, tắt dịch search nguyên văn, báo `translation_failed` đúng khi dịch lỗi) — `tests/services/test_trending_service.py`.

**Verify qua Playwright thật (API thật, không mock):** search "ẩm thực" với dịch bật → request gửi đúng `translate_keyword=true`; do máy dev chưa cấu hình key dịch nào chạy được (OpenAI 401, Google free bị rate-limit) nên dịch thất bại thật — verify đúng luồng dự phòng: tự rơi về search nguyên văn (log xác nhận Bilibili nhận đúng chuỗi gốc, không phải chuỗi rỗng/lỗi), `translation_failed=true` trả đúng, toast "Dịch từ khoá thất bại..." hiện đúng trên UI. Trang Crawl test lại sau refactor vẫn hoạt động y hệt trước (không có regression).

**Sự cố vận hành gặp lại lúc verify (biến thể mới của lỗi đã ghi ở mục "2026-09-16" phía trên)**: sau khi sửa `trending_service.py` + `trending.py`, gọi thử qua HTTP vẫn KHÔNG thấy `translation_failed` trong response dù schema đã khai báo đúng — nghi ngờ sai code hồi lâu, hoá ra `watchfiles` (`npm run dev`, có `--reload`) chỉ thật sự nạp lại sau khi sửa `schemas/trending.py` (đổi sau cùng), còn 2 file sửa trước đó (`services/trending_service.py`, `api/trending.py`) **không kích hoạt reload** vì lý do không rõ — log chỉ có đúng 1 dòng "WatchFiles detected changes" dù sửa 3 file. Gọi hàm `search_bilibili()` trực tiếp qua Python (bỏ qua HTTP) cho kết quả đúng ngay — đó là cách phát hiện ra server đang chạy code cũ. **Khắc phục**: kill toàn bộ tiến trình `npm run dev` cũ, khởi động lại sạch — hết vấn đề. Bài học thêm cho phiên sau: `--reload` không đảm bảo MỌI lần sửa file đều kích hoạt nạp lại — khi hành vi HTTP không khớp code đã sửa, thử gọi thẳng hàm qua Python (bỏ qua tầng HTTP/reload) để xác nhận code có đúng không, trước khi nghi ngờ logic sai.

**Sự cố vận hành lặp lại trong phiên:** sửa code xong mà hành vi backend không đổi — hoá ra do khởi động lại backend trước đó bằng lệnh KHÔNG có `--reload`, nên các lần sửa code sau đó không được nạp lại. Nhắc nhở cho phiên sau: nếu backend "chạy nhưng không phản ánh code mới", kiểm tra lệnh khởi động có `--reload` không trước khi nghi ngờ code sai.
