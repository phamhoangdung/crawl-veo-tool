...
- **Popup xem video tràn màn hình (2026-09-24)**: `VideoPreviewDialog` — dải gợi ý ngang có ~40 thẻ `shrink-0` làm cột grid của popup giãn theo nội dung (khung video rộng 1238px trong popup 672px → khối đen che hết màn). Sửa: `min-w-0` ở phần tử con của grid, video `w-full max-w-[calc(50vh*16/9)]`, popup `max-h-[92vh] overflow-y-auto`, `sm:max-w-2xl`. Test hồi quy dùng viewport 2556x1393 + 40 thẻ gợi ý — đã kiểm chứng test FAIL với CSS cũ, PASS với CSS mới (test với 1 thẻ / viewport nhỏ thì không bắt được lỗi). Bài học: verify UI phải đo `getBoundingClientRect` ở nhiều cỡ màn, không chỉ chụp ảnh.
- **Cột "Đã chọn" (Khám phá)**: chuyển sang bên phải, có nút ẩn/hiện (nhớ trong localStorage `discover.selectedCartVisible`).
- **Khoảng thời gian xếp hạng**: Bilibili `ranking/region` CHỈ nhận `day=3` hoặc `day=7` (đo thật: 1/30/90/365 → lỗi -400). UI có bộ chọn "3 ngày | 7 ngày" ở các tab chuyên mục; backend chặn giá trị khác bằng 422 (`RankingDays = Literal[3, 7]`). Ngày/tháng/năm thật cần đường khác (tìm theo lượt xem trong khoảng ngày đăng, endpoint `search/type` chưa kiểm chứng, kết quả không phải bảng xếp hạng chính thức).
- **Bản cài tự chứa, chạy được trên máy chưa có gì (2026-09-24, v0.1.7)** — bắt đầu từ lỗi thật `BrokenProcessPool: A child process terminated abruptly` khi "Tách lời thoại" trên bản cài. Chạy bản đóng gói trong môi trường mô phỏng máy sạch (PATH chỉ có System32, thư mục dữ liệu/cache model trống, đường dẫn cài có dấu tiếng Việt + dấu cách, cổng 8000 đang bị app khác chiếm) rồi sửa từng lỗi lộ ra:
  1. `entrypoint.py` thiếu `multiprocessing.freeze_support()` → worker của `ProcessPoolExecutor` (Windows sinh worker bằng cách chạy lại chính file .exe) chạy lại cả khởi động uvicorn rồi chết. `core/worker_pool.py` giờ cũng tự bỏ pool hỏng để lần bấm sau tạo pool mới (trước đây hỏng tới khi khởi động lại app).
  2. **ffmpeg + ffprobe đóng gói kèm** (`backend/vendor/ffmpeg/`, tải bằng `npm run desktop:fetch-ffmpeg` = `scripts/fetch-ffmpeg.mjs`, bản essentials 9.0.2 của gyan.dev host trên GitHub vì gyan.dev chỉ ~10KB/s; gitignore). Spec đưa vào `_internal/ffmpeg`, `ffmpeg_locator.ensure_ffmpeg_on_path` ưu tiên bản này. **Bản GPL (có libx264)** — phát hành thương mại phải kèm giấy phép/nguồn (LICENSE + README đã nằm cạnh binary); nếu ngại GPL thì đổi sang build LGPL và kiểm lại burn-in phụ đề.
  3. **Demucs không chạy được trong bản đóng gói**: `python -m demucs` cần interpreter rời (bản đóng gói không có) và Demucs chưa từng vào bundle. Đổi: `entrypoint.py --run-demucs ...` chạy `demucs.separate.main`, `adapters/demucs.py` gọi lại chính exe bằng cờ đó khi `is_frozen()` (vẫn là subprocess riêng). Spec `collect_all('demucs'|'speechbrain'|'faster_whisper')` (thiếu → mất `demucs/remote/*.yaml`, `silero_vad_v6.onnx`).
  4. **Phân vai người nói hỏng cả ở dev**: `torchaudio.load` từ torchaudio 2.9+ bắt buộc `torchcodec`. Đổi sang lấy mẫu thẳng từ pydub (bỏ vòng ghi wav tạm). SpeechBrain đặt `LocalStrategy.COPY` (mặc định tạo symlink → cần admin/Developer Mode trên Windows, máy người dùng thường không có).
  5. **Cổng backend không cố định 8000 nữa** (bản đóng gói): `src-tauri/src/lib.rs` xin OS một cổng rảnh, truyền qua `BACKEND_PORT`, tiêm `window.__VIEDUB_API_BASE__` vào trang trước khi frontend chạy (cửa sổ giờ tạo bằng code, `"create": false` trong `tauri.conf.json`); `frontend/src/lib/api.ts` đọc biến đó. Dev vẫn 8000. Đã verify: app thật chạy ở cổng 8395 trong lúc app cài sẵn giữ 8000, UI nối đúng backend mới.
  6. Bản cài dùng NSIS thôi (`targets: ["nsis"]`, bỏ MSI) + `webviewInstallMode: embedBootstrapper` (Windows 10 chưa có WebView2 vẫn cài được, không cần mạng lúc cài). `error_message` cũ giờ bị xoá khi bước sau chạy thành công.
  - **Đã verify (bản đóng gói, dữ liệu trống, PATH tối thiểu)**: `/health`, nhập video, tách lời thoại (Whisper tải model lần đầu), phân vai (ra đúng 2 người nói), Demucs qua cờ `--run-demucs` (tách được vocals/no_vocals). **Chưa verify**: chạy chính file `Setup.exe` trên máy Windows thật sự khác (không có VC++ runtime? — torch/PyInstaller tự kèm `vcruntime140`, nhưng chưa thử); lồng tiếng + burn-in trọn vòng qua UI trên bản cài; máy có GPU/không GPU khác nhau (Whisper cố định CPU int8).
  - **Test Setup.exe thật (2026-09-24, cùng ngày)**: (a) trích payload bằng 7-Zip + `dumpbin /dependents` mọi .exe/.dll/.pyd → phát hiện `torch_cpu.dll` cần `vcruntime140_threads.dll` (VC++ 2022 Redistributable, không có sẵn trong Windows, máy dev có nên bị che) → spec giờ kèm file này (báo lỗi build nếu máy build thiếu); kiểm lại: 0 DLL ngoài lõi Windows còn thiếu. (b) Gỡ MSI 0.1.6, chạy `Setup.exe /S` → cài vào `%LOCALAPPDATA%\VieDub Studio` (không cần admin), dữ liệu `%APPDATA%\VieDubStudio` còn nguyên, backend nghe cổng động, ffmpeg đóng gói được dùng. (c) Trên bản vừa cài: video 2 (475s) — video lỗi ban đầu — tách lời thoại xong 332 đoạn/217s; phân vai chạy được (torch nạp OK). Cách test máy sạch đúng nghĩa vẫn là Windows Sandbox (`Enable-WindowsOptionalFeature -Online -FeatureName Containers-DisposableClientVM -All`, cần admin + khởi động lại) — chưa bật.
  - **Vấn đề chất lượng phát hiện khi test (không phải lỗi đóng gói)**: phân vai với `_CLUSTER_DISTANCE_THRESHOLD = 0.7` (adapters/diarization_adapter.py) ra **63 người nói** cho video 1 người dẫn (332 đoạn, nhiều đoạn 1-2s) → lồng tiếng sẽ chia ra hàng chục giọng. Cần chỉnh ngưỡng/gộp cụm nhỏ (Phase 19).
  - **Còn phụ thuộc mạng lần đầu**: model Whisper `small` (~460MB), Demucs (~80MB), ECAPA (SpeechBrain) tải khi dùng lần đầu, không có thanh tiến trình cho Whisper. Muốn cài xong dùng offline hoàn toàn thì phải bundle model (installer to thêm ~0.6GB).
- **Lỗi "ffmpeg không có trong PATH" trên bản cài (2026-09-24)**: đã tái hiện. Mở app cài từ Explorer thì OK (PATH có `WinGet\Links`, tải+ghép xong video), nhưng tiến trình chỉ có PATH của MÁY (như khi app được mở từ trình cài đặt/dịch vụ) thì `shutil.which("ffmpeg")` = None — vì WinGet đặt ffmpeg trong PATH của NGƯỜI DÙNG. Sửa: `backend/app/core/ffmpeg_locator.py` đọc lại PATH mới từ registry (HKCU + HKLM) rồi thử các thư mục cài phổ biến (WinGet Links/Packages, Chocolatey, Scoop, `C:\ffmpeg\bin`, `%APPDATA%\VieDubStudio\bin`), thêm thư mục tìm được vào `os.environ["PATH"]` — chạy lúc khởi động và trước mỗi lần dùng ffmpeg (`ensure_ffmpeg_available`), nên cài ffmpeg xong không cần mở lại app. Verify bằng tiến trình giả lập PATH chỉ-của-máy: trước sửa `None`, sau sửa tìm thấy cả ffmpeg và ffprobe. **Vẫn chưa bundle ffmpeg** (máy chưa cài ffmpeg thì vẫn lỗi, thông báo giờ chỉ rõ `winget install Gyan.FFmpeg`). Lưu ý: khi debug đừng dùng `git stash` khi build đang chạy — nó gỡ tạm các file version do `bump-version` sửa.
